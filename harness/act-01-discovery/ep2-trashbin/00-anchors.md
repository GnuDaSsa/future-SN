# §A. 마스터 앵커 — 씬2 「중원구 여수동 가로 쓰레기통」 (단일 소스)

> EP1 2번 씬. 화풍(A-1)은 씬1과 **완전히 동일**(신카이 강화판). 인물·장소·빛·에셋만 씬2 전용.
> 컷 프롬프트의 영어 LOCK은 모두 여기서 복사 — 토씨 하나 바꾸지 않는다. 고칠 일은 여기만 고치고 재조립.
>
> **소재(기사):** 성남시가 **30년 만에 부활시킨 가로(거리) 쓰레기통** — **일반쓰레기통 75ℓ + 재활용품통 75ℓ 한 세트**,
> 버스·지하철역·대로변에 1,000조 설치(10월까지 500조 추가). 사진 캡션 위치 = **중원구 여수동**.
> **스토리:** 길에서 다 마신 컵·쓰레기를 들고 버릴 데를 못 찾던 시민이, 새로 생긴 쨍한 색 분리 쓰레기통을 발견하고 흐뭇.
...
> 씬1과 다른 새 인물(옴니버스). 한 단어만 달라져도 흔들린다. **글자 단위로 동일**하게.
...
**식별 디테일 (일관성 닻):**
- **베이지 트렌치코트** + 흰 니트 (색 대비 포인트)
- 다크브라운 **낮은 포니테일**
- **갈색 가죽 숄더백**
- 한 손 **테이크아웃 커피컵** + 다른 손 **구겨진 과자봉지(=버릴 쓰레기)**
- 흰 운동화

## A-3. LOCATION LOCK — 중원구 여수동 큰길가 인도
...
## A-4. LIGHT LOCK — 늦은 오후 황금빛 (씬1 아침과 구분, 광원 방향은 동일)
...
> ⚠️ 빛 방향은 씬1과 같이 **항상 좌상단(upper-left)**. (시간대만 오후로 달라 옴니버스 변주)

## A-5. ASSET LOCK — 가로 쓰레기통 한 세트 (컷6 원경 도입 → 컷8·9·10 본격 등장)

```
a set of Seongnam's revived street trash bins standing side by side on the sidewalk, two 75-liter modern metal bins with charcoal dark-gray boxy bodies and curved domed colored tops: the left bin bright orange for general waste with a white '일반쓰레기 / Waste' label and a single horizontal rectangular drop slot, the right bin bright lime-green for recycling with a white '재활용품 / Recycle' label, two round circular top openings and small PLASTIC/BOTTLE/CAN category icons; both with white pictograms, mounted on a tidy base near the curb
```

> 🎨 **색상(실제 기사 사진 기준 확정):** 일반쓰레기=**주황**, 재활용품=**라임그린**, 몸체=**차콜 진회색**.
> 참조 사진: `https://t1.daumcdn.net/news/202507/23/seouleconomy/20250723083834835wppw.jpg`
> 이미지 생성 시 이 사진을 레퍼런스로 첨부하면 정확도가 가장 높다. 색/형태 변경은 **이 블록만** 고치면 된다. (단일 소스)
...
| LOCK | 컷 |
|---|---|
| A-1 STYLE / A-2 CHARACTER / A-3 LOCATION / A-4 LIGHT | 전 컷 (1–10) |
| A-5 BINS | 컷 6(원경 한 줄), 8, 9, 10 |
