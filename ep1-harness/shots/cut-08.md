# 컷 08 — 컷7과 완전 동일 구도 · 눈 번쩍 ★매치컷 B

- **카메라/구도:** 컷7과 **100% 동일 프레이밍**. 눈을 번쩍 뜸 + 배경이 도로 에스컬레이터로 치환.
- **LOCK:** 컷7의 프레이밍 100% + ESCALATOR
- **VARY (이 컷에서만):** 눈을 크게 뜨고 놀란 눈썹, 계단 배경 → 도로 에스컬레이터로 치환
- **대사·SFX:** 💬 C1 "어?!" / SFX — 에스컬레이터 기계음
- **클립:** E4 (끝 프레임 = 이 컷)

> ★ **이 컷은 신규 생성이 아니다.** 컷7 키프레임을 GPT Image 2 정밀편집으로 만든다.
> "눈만 뜨고 + 배경만 에스컬레이터로 치환, 나머지 100% 동일." 전체 절차: `../02-matchcut-7-8.md`

---

## ▶ COPY BLOCK — 컷7 이미지 정밀편집 (GPT Image 2 edit)
...
## 레퍼런스 슬롯
- **편집 입력:** 컷07 확정 키프레임 (단일 이미지 편집)
- **보조 슬롯1:** §B-1 캐릭터 시트 (얼굴 고정 보강)
- **보조:** §A-5 에스컬레이터 에셋 시트(있다면)

## 연출 메모
- **머리 위치·프레임·광원은 컷7과 픽셀 단위로 정합** → 매치컷이 깔끔하게 끊긴다.
- Seedance: 컷7 키프레임 = first frame / 컷8 키프레임 = last frame, "눈 감았다 뜨는 사이 배경이 에스컬레이터로 전환" 지시.
- 놀란 눈썹 + "어?!" 싱크.
