# 컷 07 — 측면 클로즈업 · 눈 질끈 감기 ★매치컷 A

- **카메라/구도:** 측면 클로즈업 — 지쳐서 눈을 질끈 감음(포기 직전).
- **LOCK:** CHARACTER (얼굴) · LIGHT + STYLE · LOCATION
- **VARY (이 컷에서만):** 힘들어 눈을 꼭 감고 어깨가 축 처짐, 입김이 새벽 공기에 살짝 산란
- **대사·SFX:** 💬 C1 "으…"
- **클립:** E4 (시작 프레임 = 이 컷)

> ★ **매치컷의 첫 키프레임.** 이 컷이 확정되면 컷8은 *새로 생성하지 않고* 이 이미지를 정밀편집한다.
> 상세 절차: `../02-matchcut-7-8.md`
...
## 연출 메모
- 광원 **upper-left** 확인.
- **프레이밍·머리 위치·광원을 고정**할 것 — 컷8이 이 이미지에서 픽셀 단위로 파생되어야 매치컷이 깔끔하다.
- 배경은 아직 **계단길** (에스컬레이터 아님).
