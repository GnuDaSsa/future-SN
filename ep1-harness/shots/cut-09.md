# 컷 09 — 측면 풀숏 / 에스컬레이터 탑승

- **카메라/구도:** 측면 풀숏, 도로 에스컬레이터 탑승(상행).
- **LOCK:** CHARACTER · ESCALATOR · LIGHT + STYLE · LOCATION
- **VARY (이 컷에서만):** 고무 난간 잡고 스르륵 올라감, 빨간 가방 들썩, 신난 표정, 미끄러지는 배경
- **대사·SFX:** SFX — 기계음, 바람
- **클립:** E5 (멀티샷: 탑승 → 크레인 리빌)
...
## 연출 메모
- 광원 **upper-left** 확인. 채도 소폭 상승 OK(연출 의도) — 색온도·광원방향 유지.
- 양손은 난간, 빨간 가방은 등에 — 지친 표정에서 신난 표정으로 전환.
